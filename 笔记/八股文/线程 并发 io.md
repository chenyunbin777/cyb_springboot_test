...
- 总结
总线嗅探是**硬件层面**上的一种缓存一致性协议，用于确保多个处理器缓存中数据的一致性。这对 Java 内存模型的实现至关重要，特别是在保证多线程中共享变量的可见性时。
总线风暴则是缓存一致性协议中的一种极端情况，频繁的缓存一致性事务会占用大量总线带宽，影响系统性能。
在 Java 中，volatile 依赖于总线嗅探来确保数据的可见性，但不保证操作的原子性，锁机制（如 synchronized 和 ReentrantLock）则提供了更全面的内存可见性和原子性保障。

原文链接：https://blog.csdn.net/qq_41520636/article/details/143171237
- 3 有序性：
