Redis在互联网技术存储方面使用如此广泛，几乎所有的后端技术面试官都要在Redis的使用和原理方面对小伙伴们进行各种刁难。
...
- 1 分布式锁过期时间到了，但是代码程序还没有执行完的问题

- 2 Redis集群主从模型中，主节点挂了之后，哨兵从 从节点选举出来一个master，但是分布式锁数据还没有及时同步到从节点，这时
任务过来加锁就会有两个锁数据